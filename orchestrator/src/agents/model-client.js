...
要求：
1. 提取页面上所有带编号的条目，不要遗漏。
2. 题号必须从 1 开始连续递增。如果试卷分多个板块且每个板块重新编号（如"单词 1-9"然后"词块 1-13"），请统一编为连续编号（单词1=1, 单词9=9, 词块1=10, 词块2=11...）。
3. 按题号从小到大排列，不能跳号。
4. 每题必须有 confidence 字段，不确定的题给低于 0.6 的值。
5. knowledge_tag 根据答案内容推断即可。
6. 若某题答案无法识别，correct_answer 留空，confidence 设为 0。
${safeHint ? `提示信息：${safeHint}` : ''}`
  ...
  async ocrStudentAnswers({ imagePaths, questionCount }) {
    if (!imagePaths || imagePaths.length === 0) {
...
${questionCount ? `\n本次过关单共 ${questionCount} 题，请严格输出 ${questionCount} 个答案（1:... 到 ${questionCount}:...）。` : ''}
要求：
...
5. 题号必须从 1 开始连续递增，不能跳号。如果试卷分多个板块且每个板块重新编号（如"单词 1-9"然后"词块 1-13"），请统一编为连续编号（单词1=1, 单词9=9, 词块1=10, 词块2=11...）。
6. 每道题都必须输出，没有手写答案的题也要输出"?"，确保题号连续不中断。${questionCount ? `\n7. 只读取前 ${questionCount} 道填空题的手写答案，忽略页面上其他板块（如句子、词形变化等非填空内容）。` : ''}
8. 不要输出任何解释、JSON 或 markdown，只输出一行紧凑文本。`
      },
    ];
